// ==================== THEME ENGINE ====================
        ...
        window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', (e) => {
            if (ThemeEngine.currentTheme === 'system') { ThemeEngine.apply('system'); }
        });
        ...
        function showSubPage(pageName) {
            ...
            if (subpage) { subpage.classList.add('active'); subpage.scrollTop = 0; }
        ...
        const AudioEngine = {
            ...
            playSave() {
                ...
                const osc = this.ctx.createOscillator();
                const gain = this.ctx.createGain();
                osc.type = 'sine'; osc.frequency.setValueAtTime(550, t);
        ...
        function toggleSave(btn, e) {
            e.stopPropagation(); AudioEngine.init();
            const isSaved = btn.classList.contains('saved');
            if (isSaved) { btn.classList.remove('saved'); AudioEngine.playUnsave(); haptic('light'); }
            else { btn.classList.add('saved', 'just-saved'); AudioEngine.playSave(); haptic('save'); setTimeout(() => btn.classList.remove('just-saved'), 450); }
        }
        ...
        // ==================== PROPERTY DATA (SALES PRICES) ====================
        const bestMatches = [
            {
                id: 1, image: 'h2.jpg', type: '3 Bedroom Duplex',
                location: 'Osongama Estate, Uyo', trustScore: 92,
                price: '₦45,000,000',
                indicators: [
                    { label: 'Title Verified', positive: true },
                    { label: 'Ownership Confirmed', positive: true }
                ], saved: false
            },
            {
                id: 2, image: 'h3.jpg', type: '2 Bedroom Apartment',
                location: 'Ewet Housing, Uyo', trustScore: 88,
                price: '₦28,000,000',
                indicators: [
                    { label: 'Survey Complete', positive: true },
                    { label: 'Deed Authentic', positive: true }
                ], saved: true
            },
            {
                id: 3, image: 'h4.jpg', type: '4 Bedroom Terrace',
                location: 'GRA Phase 2, Uyo', trustScore: 95,
                price: '₦55,000,000',
                indicators: [
                    { label: '24/7 Security', positive: true },
                    { label: 'Transfer Ready', positive: true }
                ], saved: false
            }
        ];
        ...
        const newlyVerified = [
            {
                id: 4, image: 'h5.jpg', title: '1 Bedroom Apartment',
                location: 'UniUyo Area, Uyo', time: 'Listed today',
                trustScore: 91, price: '₦18,500,000'
            },
            {
                id: 5, image: 'h6.jpg', title: '2 Bedroom Flat',
                location: 'GRA, Uyo', time: 'Listed 2 days ago',
                trustScore: 87, price: '₦42,000,000'
            },
            {
                id: 6, image: 'h7.jpg', title: 'Duplex (3 Bed)',
                location: 'Ibom Leisure, Uyo', time: 'Listed 1 week ago',
                trustScore: 89, price: '₦65,000,000'
            }
        ];

        const savedProperties = [
            {
                id: 7, image: 'h3.jpg', title: '2 Bedroom Apartment',
                location: 'Ewet Housing, Uyo', time: 'Saved 2 days ago',
                trustScore: 88, price: '₦28,000,000'
            },
            {
                id: 8, image: 'h4.jpg', title: '4 Bedroom Terrace',
                location: 'GRA Phase 2, Uyo', time: 'Saved 1 week ago',
                trustScore: 95, price: '₦55,000,000'
            }
        ];
        ...
        function renderPropertyCard(property) {
            ...
            card.addEventListener('click', (e) => {
                ...
                createRipple(e, card);
                AudioEngine.init(); AudioEngine.playTap(); haptic('light');
                setTimeout(() => { window.location.href = 'bpd.html'; }, 180);
            });
        ...
        // ==================== QUALIFICATION METER ====================
        const qualMeter = document.getElementById('qualMeter');
        if (qualMeter) {
            qualMeter.addEventListener('click', (e) => {
                createRipple(e, qualMeter);
                AudioEngine.init(); AudioEngine.playTap(); haptic('light');
                switchTab('finance');
            });
        }
        ...
        if (searchInput) { searchInput.addEventListener('focus', () => { AudioEngine.init(); }); }
        ...
        // ==================== FINANCE SCORE ANIMATION ====================
        function animateFinanceScore() {
            const el = document.getElementById('financeScoreNum');
            if (!el) return;
            let current = 0; const target = 78;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ThemeEngine.init();
            loadProperties(); loadVerified(); loadSaved();
            let financeAnimated = false;
            const financeTab = document.getElementById('tab-finance');
            const observer = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    if (mutation.target.classList.contains('active') && !financeAnimated) {
                        financeAnimated = true; setTimeout(animateFinanceScore, 300);
                    }
                });
            });
            if (financeTab) { observer.observe(financeTab, { attributes: true, attributeFilter: ['class'] }); }
        });
